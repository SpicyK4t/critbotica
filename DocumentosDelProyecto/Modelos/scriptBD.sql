...
-- Fri 25 Sep 2015 02:44:43 AM CDT
...
-- -----------------------------------------------------
-- Placeholder table for view `crit`.`stockminimo_alcanzado`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `crit`.`stockminimo_alcanzado` (`id` INT, `nombre_distintivo` INT, `nombre_generico` INT, `presentacion` INT, `stock_minimo` INT, `user_id` INT, `grupo` INT, `habilitado` INT, `imagen_medicamento` INT, `existencia` INT);

-- -----------------------------------------------------
-- Placeholder table for view `crit`.`por_caducar`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `crit`.`por_caducar` (`id` INT, `nombre_distintivo` INT, `nombre_generico` INT, `presentacion` INT, `stock_minimo` INT, `user_id` INT, `grupo` INT, `habilitado` INT, `imagen_medicamento` INT);
...
-- -----------------------------------------------------
-- View `crit`.`stockminimo_alcanzado`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `crit`.`stockminimo_alcanzado` ;
DROP TABLE IF EXISTS `crit`.`stockminimo_alcanzado`;
USE `crit`;
CREATE  OR REPLACE VIEW `stockminimo_alcanzado` AS SELECT *, (SELECT existencia FROM `estadomedicamentos` WHERE id = medicamentos.id) as existencia FROM `medicamentos` WHERE (SELECT existencia FROM `estadomedicamentos` WHERE id = medicamentos.id) < stock_minimo
;

-- -----------------------------------------------------
-- View `crit`.`por_caducar`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `crit`.`por_caducar` ;
DROP TABLE IF EXISTS `crit`.`por_caducar`;
USE `crit`;
CREATE  OR REPLACE VIEW `por_caducar` AS SELECT * FROM `medicamentos` WHERE (SELECT caducidad FROM entradas WHERE medicamentos.id = entradas.medicamento_id LIMIT 1) BETWEEN NOW() AND NOW() + INTERVAL 4 MONTH
;
